...
public class MeshData
{
    ...
    public Vector3[] CalculateNormals()
    {
        ...
        int borderCount = borderTriangles.Length / 3;
        for (int i = 0; i < borderCount; i++)
        {
            int index = i * 3;
            int a = borderTriangles[index];
            int b = borderTriangles[index + 1];
            int c = borderTriangles[index + 2];

            Vector3 normal = SurfaceNormalFromIndices(a, b, c);
            if (a >= 0) vertexNormals[a] += normal;
            if (b >= 0) vertexNormals[b] += normal;
            if (c >= 0) vertexNormals[c] += normal;
        }
